...
from . import activations, blocks
from .bs_roformer5 import TransformerBlock
from .module import EncoderBlock, ResLSTM, WNConv1d
...
class Codec_oobleck_Transformer(nn.Module):
    ...
    def dilated_unit(self, hidden_dim, dilation):
        return blocks.DilatedConvolutionalUnit(
            hidden_dim, dilation, kernel_size=3, activation=nn.ReLU, normalization=utils.weight_norm
        )

    def downsampling_unit(self, input_dim: int, output_dim: int, stride: int):
        return blocks.DownsamplingUnit(
            input_dim, output_dim, stride, nn.ReLU, normalization=utils.weight_norm
        )
