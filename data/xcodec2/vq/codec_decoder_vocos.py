...
class UpSamplerBlock(nn.Module):
    ...
    def __init__(
        ...
        assert len(self.kernel_sizes) == len(self.upsample_factors), (
            "kernel_sizes and upsample_factors must have the same length"
        )

        self.upsample_layers = nn.ModuleList()
        self.resnet_blocks = nn.ModuleList()
        self.out_proj = nn.Linear(
            self.in_channels // (2 ** len(self.upsample_factors)), self.in_channels, bias=True
        )
...
class CodecDecoderVocos(nn.Module):
    def __init__(
        ...
        if (self.sample_rate % 50) != 0 or (
            ...
            raise ValueError(
                f"sample_rate {self.sample_rate}, hop_length {self.hop_length}, upsample_factors {upsample_factors} not aligned!"
            )

        self.head = ISTFTHead(
            dim=hidden_dim, n_fft=self.hop_length * 4, hop_length=self.hop_length, padding="same"
        )
