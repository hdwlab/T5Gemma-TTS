...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        # q, k, v: (b, h, t, d)
        q, k, v = rearrange(self.c_attn(x), "b t (r h d) -> r b t h d", r=3, h=self.n_heads)
        # q, k, v: (b, t, h, d)

        q = self.rotary_embed(q)
        k = self.rotary_embed(k)

        # # rearrange to (b, h, t, d) for flash attention
        q, k, v = (rearrange(t, "b t h d -> b h t d") for t in (q, k, v))
